...
suite('Flink SQL Grammar Test Suite', () => {
    ...
    const exampleDir = path.join(projectRoot, 'SparkSQL-Example');
    ...
    test('Error SQL files should fail parsing', () => {
        ...
        const files = fs.readdirSync(errorDir).filter(file => file.endsWith('.ssql'));
    ...
    test('Correct SQL files should pass parsing', () => {
        ...
        correctDirs.forEach(dir => {
            ...
            const files = fs.readdirSync(correctDir).filter(file => file.endsWith('.ssql'));
