...
loadStatement:
	KW_LOAD KW_DATA KW_LOCAL? KW_INPATH StringLiteral KW_OVERWRITE? KW_INTO KW_TABLE
		tableOrPartition inputFileFormat?;
...
replDumpStatement:
	KW_REPL KW_DUMP dbPolicy = replDbPolicy (
...
replLoadStatement:
	KW_REPL KW_LOAD sourceDbPolicy = replDbPolicy (
		KW_INTO dbName = id_
	)? (KW_WITH replConf = replConfigs)?;

replConfigs: LPAREN replConfigsList RPAREN;

replConfigsList: keyValueProperty (COMMA keyValueProperty)*;

replTableLevelPolicy:
	replTablesIncludeList = StringLiteral (
		DOT replTablesExcludeList = StringLiteral
	)?;

replStatusStatement:
	KW_REPL KW_STATUS dbName = id_ (
		KW_WITH replConf = replConfigs
	)?;
...
tabPartColTypeExpr: tableName partitionSpec? extColumnName?;
...
db_schema: KW_DATABASE | KW_SCHEMA;

showStatement:
	KW_SHOW (KW_DATABASES | KW_SCHEMAS) (
		KW_LIKE showStmtIdentifier
	)?
	| KW_SHOW isExtended = KW_EXTENDED? KW_TABLES (
		from_in db_name = id_
	)? filter = showTablesFilterExpr?
	| KW_SHOW KW_VIEWS (from_in db_name = id_)? (
		KW_LIKE showStmtIdentifier
		| showStmtIdentifier
	)?
	...
	| KW_SHOW KW_SORTED? KW_COLUMNS from_in tableName (
		from_in db_name = id_
	)? (KW_LIKE showStmtIdentifier | showStmtIdentifier)?
	| KW_SHOW KW_FUNCTIONS (KW_LIKE showFunctionIdentifier)?
	| KW_SHOW KW_PARTITIONS tabName = tableName partitionSpec? whereClause? orderByClause?
		limitClause?
	| KW_SHOW KW_CREATE (
		db_schema db_name = id_
	...
	| KW_SHOW KW_COMPACTIONS (
		compactionId
		| db_schema dbName = id_ compactionPool? compactionType? compactionStatus? orderByClause?
			limitClause?
		| parttype = partTypeExpr? compactionPool? compactionType? compactionStatus? orderByClause?
			limitClause?
	)
...
lockDatabase: KW_LOCK db_schema dbName = id_ lockMode;

lockMode: KW_SHARED | KW_EXCLUSIVE;

unlockStatement: KW_UNLOCK KW_TABLE tableName partitionSpec?;

unlockDatabase: KW_UNLOCK db_schema dbName = id_;

createRoleStatement: KW_CREATE KW_ROLE roleName = id_;

dropRoleStatement: KW_DROP KW_ROLE roleName = id_;
...
setRole: KW_SET KW_ROLE ( all = KW_ALL | none = KW_NONE | id_);

showGrants:
	KW_SHOW KW_GRANT principalName? (
		KW_ON privilegeIncludeColObject
	)?;

showRolePrincipals: KW_SHOW KW_PRINCIPALS roleName = id_;

privilegeIncludeColObject: KW_ALL | privObjectCols;
...
viewPartition:
	KW_PARTITIONED KW_ON (
		LPAREN columnNameList
		| KW_SPEC LPAREN spec = partitionTransformSpec
	) RPAREN;

viewOrganization: viewClusterSpec | viewComplexSpec;
...
createMaterializedViewStatement:
	KW_CREATE KW_MATERIALIZED KW_VIEW ifNotExists? name = tableName rewriteDisabled? tableComment?
		viewPartition? viewOrganization? tableRowFormat? tableFileFormat? tableLocation?
...
createScheduledQueryStatement:
	KW_CREATE KW_SCHEDULED KW_QUERY name = id_ scheduleSpec executedAsSpec? enableSpecification?
		definedAsSpec;
...
executedAsSpec: KW_EXECUTED KW_AS executedAs = StringLiteral;

definedAsSpec: KW_DEFINED? KW_AS statement;
...
createTablePartitionColumnSpec: columnName (COMMA columnName)*;

partitionTransformSpec:
	columnNameTransformConstraint (
		COMMA columnNameTransformConstraint
	)*;
...
tableImplBuckets: KW_CLUSTERED KW_INTO num = Number KW_BUCKETS;

tableSkewed:
	KW_SKEWED KW_BY LPAREN skewedCols = columnNameList RPAREN KW_ON LPAREN skewedValues =
		skewedValueElement RPAREN storedAsDirs?;
...
enableSpecification: KW_ENABLE | KW_DISABLE;

validateSpecification: KW_VALIDATE | KW_NOVALIDATE;

enforcedSpecification: KW_ENFORCED | KW_NOT KW_ENFORCED;

relySpecification: KW_RELY | KW_NORELY;

createConstraint: (KW_CONSTRAINT constraintName = id_)? tableLevelConstraint constraintOptsCreate?;
...
createForeignKey: (KW_CONSTRAINT constraintName = id_)? KW_FOREIGN KW_KEY fkCols =
		columnParenthesesList KW_REFERENCES tabName = tableName parCols = columnParenthesesList
...
orderSpecification: KW_ASC | KW_DESC;

nullOrdering: KW_NULLS (KW_FIRST | KW_LAST);
...
orderSpecificationRewrite: KW_ASC | KW_DESC;
...
tableConstraint: createForeignKey | createConstraint;

columnNameTypeConstraint:
	colName = id_ colType columnConstraint? (
		KW_COMMENT comment = StringLiteral
	)?;

columnConstraint: foreignKeyConstraint | colConstraint;

foreignKeyConstraint: (KW_CONSTRAINT constraintName = id_)? KW_REFERENCES tabName = tableName LPAREN
		colName = columnName RPAREN constraintOptsCreate?;

colConstraint: (KW_CONSTRAINT constraintName = id_)? columnConstraintType constraintOptsCreate?;

alterColumnConstraint:
	alterForeignKeyConstraint
	| alterColConstraint;

alterForeignKeyConstraint: (KW_CONSTRAINT constraintName = id_)? KW_REFERENCES tabName = tableName
		LPAREN colName = columnName RPAREN constraintOptsAlter?;

alterColConstraint: (KW_CONSTRAINT constraintName = id_)? columnConstraintType constraintOptsAlter?;
...
defaultVal: constant | function_ | castExpression;

tableConstraintType: KW_PRIMARY KW_KEY | KW_UNIQUE;
...
primitiveType:
	...
	//| KW_TIMESTAMP KW_WITH KW_TIME KW_ZONE Uncomment to allow intervals as table column types |
	// KW_INTERVAL KW_YEAR KW_TO KW_MONTH | KW_INTERVAL KW_DAY KW_TO KW_SECOND
	| KW_STRING
	| KW_BINARY
	| KW_DECIMAL (
...
setOperator: (KW_UNION | KW_INTERSECT | KW_EXCEPT | KW_MINUS) (
		KW_ALL
		| KW_DISTINCT
	)?;

queryStatementExpression
/* Would be nice to do this as a gated semantic perdicate But the predicate gets pushed as a
 lookahead decision. Calling rule doesnot know about topLevel
 */: w = withClause? queryStatementExpressionBody;

queryStatementExpressionBody: fromStatement | regularBody;

withClause: KW_WITH cteStatement (COMMA cteStatement)*;
...
singleFromStatement: fromClause b += body+;

/*
 The valuesClause rule below ensures that the parse tree for "insert into table FOO values
 (1,2),(3,4)" looks the same as "insert into table FOO select a,b from (values(1,2),(3,4)) as
 BAR(a,b)" which itself is made to look very similar to the tree for "insert into table FOO select
 a,b from BAR".
 */
regularBody:
...
atomSelectStatement:
	s = selectClause f = fromClause? w = whereClause? g = groupByClause? h = havingClause? win =
		window_clause? q = qualifyClause?
	| LPAREN selectStatement RPAREN
	| valuesSource;

selectStatement:
	a = atomSelectStatement set = setOpSelectStatement? o = orderByClause? c = clusterByClause? d =
		distributeByClause? where = whereClause? sort = sortByClause? l = limitClause?;

setOpSelectStatement: (u = setOperator b = atomSelectStatement)+;

selectStatementWithCTE: w = withClause? selectStatement;

body:
	insertClause selectClause lateralView? whereClause? groupByClause? havingClause? window_clause?
		qualifyClause? orderByClause? clusterByClause? distributeByClause? sortByClause? limitClause
		?
...
insertClause:
	KW_INSERT (
		...
		| KW_INTO KW_TABLE? tableOrPartition (
...
limitClause:
	KW_LIMIT (
		(offset = Number COMMA)? num = Number
		| num = Number KW_OFFSET offset = Number
	);

/**
 DELETE FROM <tableName> WHERE ...;
 */
deleteStatement: KW_DELETE KW_FROM tableName whereClause?;
...
transactionAccessMode: KW_READ (KW_ONLY | KW_WRITE);
...
commitStatement: KW_COMMIT KW_WORK?;

rollbackStatement: KW_ROLLBACK KW_WORK?;
...
abortTransactionStatement: KW_ABORT KW_TRANSACTIONS Number+;

abortCompactionStatement: KW_ABORT KW_COMPACTIONS Number+;

/*
 BEGIN SQL Merge statement
 */
mergeStatement:
	KW_MERGE QUERY_HINT? KW_INTO tableName (KW_AS? id_)? KW_USING joinSourcePart KW_ON expression
		whenClauses;
/*
 Allow 0,1 or 2 WHEN MATCHED clauses and 0 or 1 WHEN NOT MATCHED Each WHEN clause may have AND
 <boolean predicate>. If 2 WHEN MATCHED clauses are present, 1 must be UPDATE the other DELETE and
 the 1st one must have AND <boolean predicate>
 */
whenClauses: (whenMatchedAndClause | whenMatchedThenClause)* whenNotMatchedClause?;

whenNotMatchedClause:
	KW_WHEN KW_NOT KW_MATCHED (KW_AND expression)? KW_THEN KW_INSERT targetCols =
		columnParenthesesList? KW_VALUES valueRowConstructor;
...
updateOrDelete: KW_UPDATE setColumnsClause | KW_DELETE;
/*
 END SQL Merge statement
 */

killQueryStatement: KW_KILL KW_QUERY StringLiteral+;

/*
 BEGIN SHOW COMPACTIONS statement
 */
compactionId: KW_COMPACT_ID EQUAL compactId = Number;

compactionPool: KW_POOL poolName = StringLiteral;

compactionType: KW_TYPE compactType = StringLiteral;

compactionStatus: KW_STATUS status = StringLiteral;
...
alterStatementSuffixAddCol: (add = KW_ADD | replace = KW_REPLACE) KW_COLUMNS LPAREN
...
alterStatementSuffixSerdeProperties:
	KW_SET (
		KW_SERDE serdeName = StringLiteral (
...
compactPool: KW_POOL poolName = StringLiteral;
...
alterStatementSuffixExecute:
	KW_EXECUTE (
		KW_ROLLBACK LPAREN rollbackParam = (
			StringLiteral
			| Number
		)
 ...
 */
createTableStatement:
	KW_CREATE temp = KW_TEMPORARY? trans = KW_TRANSACTIONAL? ext = KW_EXTERNAL? KW_TABLE ifNotExists
		? name = tableName (
...
createDataConnectorStatement:
	KW_CREATE KW_DATACONNECTOR ifNotExists? name = id_ dataConnectorType dataConnectorUrl
		dataConnectorComment? (
		KW_WITH KW_DCPROPERTIES dcprops = dcProperties
	)?;

dataConnectorComment: KW_COMMENT comment = StringLiteral;

dataConnectorUrl: KW_URL url = StringLiteral;

dataConnectorType: KW_TYPE dcType = StringLiteral;
...
aliasList: id_ (COMMA id_)*;

/**
 Rules for parsing fromClause from [col1, col2, col3] table1, [col4, col5] table2
 */
...
lateralView:
	...
	| COMMA? KW_LATERAL (
		...
		| KW_TABLE LPAREN valuesClause RPAREN KW_AS? tableAlias (
...
splitSample:
	KW_TABLESAMPLE LPAREN (
		Number (KW_PERCENT | KW_ROWS)
		| ByteLengthLiteral
	) RPAREN;
...
tableName: db = id_ DOT tab = id_ (DOT meta = id_)? | tab = id_;

viewName: (db = id_ DOT)? view = id_;
...
/**
 in support of SELECT * FROM (VALUES(1,2,3),(4,5,6),...) as FOO(a,b,c) and INSERT INTO <table>
 (col1,col2,...) VALUES(...),(...),... INSERT INTO <table> (col1,col2,...) SELECT * FROM
...
/*
 This represents a clause like this: TABLE(VALUES(1,2),(2,3)) as VirtTable(col1,col2)
 */
virtualTableSource:
	KW_TABLE LPAREN valuesClause RPAREN KW_AS? tableAlias (
		LPAREN id_ (COMMA id_)*
	)? RPAREN;

/*
 Rules for parsing selectClause select a,b,c ...
 */
selectClause:
	KW_SELECT QUERY_HINT? (
		all_distinct? selectList
		| KW_TRANSFORM selectTrfmClause
	)
	| trfmClause;

all_distinct: KW_ALL | KW_DISTINCT;
...
selectItem:
	...
	| (
		expression (
			KW_AS? id_
			| KW_AS LPAREN id_ (COMMA id_)* RPAREN
		)?
	);

trfmClause: (KW_MAP | KW_REDUCE) selectExpressionList rowFormat recordWriter KW_USING StringLiteral
		(
		KW_AS (
			LPAREN (aliasList | columnNameTypeList) RPAREN
			| aliasList
...
selectExpression: tableAllColumns | expression;
...
window_frame_boundary: (KW_UNBOUNDED | Number) (
		KW_PRECEDING
		| KW_FOLLOWING
	)
	| KW_CURRENT KW_ROW;
...
rollupStandard: (rollup = KW_ROLLUP | cube = KW_CUBE) LPAREN expression (
		COMMA expression
	)* RPAREN;
...
expressionPart: (COMMA expressionOrDefault)+;

expressionOrDefault: defaultValue | expression;

/**
 Parses comma separated list of expressions with optionally specified aliases. <expression>
 [<alias>] [, <expression> [<alias>]]
 */
firstExpressionsWithAlias:
	first = expression KW_AS? colAlias = id_? (
		COMMA expressionWithAlias
	)*;

/** Parses expressions which may have alias. If alias is not specified generate one.
 */
expressionWithAlias: expression KW_AS? alias = id_?;
...
function_:
	...
	| functionName LPAREN (
		...
		// No null treatment: SELECT first_value(b) OVER (<window spec>) Standard null treatment
		// spec: SELECT first_value(b) IGNORE NULLS OVER (<window spec>)
...
null_treatment: KW_RESPECT KW_NULLS | KW_IGNORE KW_NULLS;
...
castExpression:
	KW_CAST LPAREN expression KW_AS toType = primitiveType (
...
floorExpression:
	KW_FLOOR LPAREN expression (
		KW_TO floorUnit = floorDateQualifiers
	)? RPAREN;
...
prepareStmtParam: p = parameterIdx;
...
dateLiteral: KW_DATE StringLiteral | KW_CURRENT_DATE;
...
intervalValue: StringLiteral | Number;
...
precedenceAmpersandExpression:
	precedenceConcatenateExpression (
		precedenceAmpersandOperator precedenceConcatenateExpression
	)*;
...
precedenceRegexpOperator: KW_LIKE | KW_RLIKE | KW_REGEXP;
...
quantifierType: KW_ANY | KW_SOME | KW_ALL;
...
booleanValueTok: KW_TRUE | KW_FALSE;
...
partitionVal: id_ (EQUAL constant)?;
...
partitionSelectorOperator: KW_LIKE | subQuerySelectorOperator;
...
id_: Identifier | nonReserved;

functionIdentifier: id_ (DOT fn = id_)?;
...
/**
 Here is what you have to do if you would like to add a new keyword. Note that non reserved keywords
 are basically the keywords that can be used as identifiers. (1) Add a new entry to HiveLexer, e.g.,
 KW_TRUE : 'TRUE'; (2) If it is reserved, you do NOT need to change IdentifiersParser.g because all
 the KW_* are automatically not only keywords, but also reserved keywords. However, you need to add
 a test to TestSQL11ReservedKeyWordsNegative.java. Otherwise it is non-reserved, you need to put
 them in the nonReserved list below. If you are not sure, please refer to the SQL2011 column in
...
hintName: KW_MAPJOIN | KW_SEMI | KW_STREAMTABLE | KW_PKFK_JOIN;

hintArgs: hintArgName (COMMA hintArgName)*;

hintArgName: Identifier | Number | KW_NONE;
...
executeParamList: constant (COMMA constant)*;
...
rpAssignList: rpAssign (COMMA rpAssign)*;

rpUnassign: KW_QUERY_PARALLELISM | KW_DEFAULT KW_POOL;

rpUnassignList: rpUnassign (COMMA rpUnassign)*;
...
activate: KW_ACTIVATE withReplace?;
...
/** It might make sense to make this more generic, if something else could be enabled/disabled. For
 now, it's only used for WM. Translate into another form of an alter statement.
 */
globalWmStatement: (KW_ENABLE | KW_DISABLE) KW_WORKLOAD KW_MANAGEMENT;
...
/*
 The rules triggerOrExpression and triggerAndExpression are not being used right now. Only >
 operator is supported, this should be changed if logic in ExpressionFactory changes.
 */
triggerOrExpression:
...
triggerLiteral: Number | StringLiteral;

comparisionOperator: GREATERTHAN;

triggerActionExpression: KW_KILL | KW_MOVE KW_TO poolPath;

triggerActionExpressionStandalone: triggerActionExpression EOF;

createTriggerStatement:
	KW_CREATE KW_TRIGGER rpName = id_ DOT triggerName = id_ KW_WHEN triggerExpression KW_DO
...
alterTriggerStatement:
	KW_ALTER KW_TRIGGER rpName = id_ DOT triggerName = id_ (
		...
		| (KW_ADD KW_TO | KW_DROP KW_FROM) (
			KW_POOL poolName = poolPath
			| KW_UNMANAGED
		)
	);
...
poolAssignList: poolAssign (COMMA poolAssign)*;
...
dropPoolStatement: KW_DROP KW_POOL rpName = id_ DOT poolPath;

createMappingStatement:
	KW_CREATE mappingType = (KW_USER | KW_GROUP | KW_APPLICATION) KW_MAPPING name = StringLiteral
		KW_IN rpName = id_ (KW_TO path = poolPath | unmanaged) (
		KW_WITH KW_ORDER order = Number
	)?;

alterMappingStatement:
	KW_ALTER mappingType = (KW_USER | KW_GROUP | KW_APPLICATION) KW_MAPPING name = StringLiteral
		KW_IN rpName = id_ (KW_TO path = poolPath | unmanaged) (
		KW_WITH KW_ORDER order = Number
	)?;

dropMappingStatement:
	KW_DROP mappingType = (KW_USER | KW_GROUP | KW_APPLICATION) KW_MAPPING name = StringLiteral
		KW_IN rpName = id_;
...
KW_CURRENT_TIMESTAMP:
	C U R R E N T UNDERLINE_SIGN T I M E S T A M P;
...
KW_EXPIRE_SNAPSHOTS:
	E X P I R E UNDERLINE_SIGN S N A P S H O T S;
...
KW_QUERY: Q U E R Y;
KW_QUERY_PARALLELISM:
	Q U E R Y UNDERLINE_SIGN P A R A L L E L I S M;
KW_RANGE: R A N G E;
KW_READ: R E A D;
...
KW_SCHEDULING_POLICY:
	S C H E D U L I N G UNDERLINE_SIGN P O L I C Y;
...
KW_SETS: S E T S;
KW_SET_CURRENT_SNAPSHOT:
	S E T UNDERLINE_SIGN C U R R E N T UNDERLINE_SIGN S N A P S H O T;
